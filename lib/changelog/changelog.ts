...
export async function addChangelogEntryForCommit(
    ...
    for (const commit of push.commits) {
        ...
        const entry: ChangelogEntry = {
            ...
            url: commit.url,
            authors: [
                { login: commit.author.login, name: commit.author.name, email: commit.author.emails?.[0]?.address },
            ],
            qualifiers,
        };
...
export function addEntryToChangelog(
    ...
    const line = `-   ${prefix}${_.upperFirst(title)} [${entry.label}](${entry.url})${
        entry.authors?.length > 0
            ? ` by ${entry.authors.map(a => `[@${a.login}](https://github.com/${a.login})`).join(", ")}`
            : ""
    }`;
